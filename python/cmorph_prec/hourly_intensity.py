...
#%% 日变化的相位
    seasons = ['MAM', 'JJA', 'SON', 'DJF']
    proj = ccrs.PlateCarree()
    fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(10,6), dpi=300, subplot_kw={'projection':proj})
    ...
    for i, season in enumerate(seasons):
        ...
        prec_hourly_cli.coords['hour'] = prec_hourly_cli.coords['hour'].roll(hour=-6) # 转换成当地时
        prec_phase = prec_hourly_cli.argmax(dim='hour').load()

        angle = - (2*np.pi/24 * prec_phase - np.pi/2)
        u, v = np.cos(angle), np.sin(angle)
        colors = convert_color(prec_phase)

        lon, lat = u.coords['lon'], u.coords['lat']

        print('>> visualization <<')
        ax = axes[rows[i]][cols[i]]

        ax.set_extent([73, 105, 25, 40], crs=proj)
        add_TP(ax)
        add_lakes(ax)
        ### 去除地图边框
        ax.background_patch.set_visible(False)
        ...
        for k in range(4):
            ...
            q = ax.quiver(LON[::span, ::span], LAT[::span, ::span], u_mask, v_mask, color=c[k], transform=proj, scale=70, width=0.0018, pivot='mid', headwidth=5, headlength=4)
