...
#%%
    ### 取0 UTC和6 UTC的平均
    # tsk1_lake_daily = tsk1_lake_mean.sel(Time=tsk1_lake_mean.Time.dt.hour.isin([0, 6])).resample(Time='D').mean()
    # tsk2_lake_daily = tsk2_lake_mean.sel(Time=tsk2_lake_mean.Time.dt.hour.isin([0, 6])).resample(Time='D').mean()

    tsk1_lake_daily = tsk1_lake_mean.sel(Time=tsk1_lake_mean.Time.dt.hour.isin([6]))
    tsk2_lake_daily = tsk2_lake_mean.sel(Time=tsk2_lake_mean.Time.dt.hour.isin([6]))
    ...
    fig.savefig('/home/zzhzhao/code/python/wrf-test-10/fig/lswt.jpg', dpi=300, bbox_inches='tight', pad_inches=0.1)
